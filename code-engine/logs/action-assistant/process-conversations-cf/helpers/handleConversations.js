...
function handleConversations(
  ...
  return {
    ...
    transfered:
      existantConversationData?.TRANSFERED ?? checkIfTransfered(log) ? 1 : 0,
    relevance:
...
function findFeedBack(log, feedbackVar) {
  const feedbackTurnEvent = log.response.output.debug?.turn_events;

  if (feedbackTurnEvent.length > 0) {
    return handleFeedback(feedbackTurnEvent, feedbackVar);
  } else return 0;
}

function handleFeedback(feedbackTurnEvent, feedbackVar) {
  for (let turnEvent of feedbackTurnEvent) {
    if (turnEvent.action_variables)
      var possibleValues = Object.entries(turnEvent.action_variables).find(
        ([key, value]) => feedbackVar.some((search) => search == key)
      );
    if (possibleValues) break;
  }
