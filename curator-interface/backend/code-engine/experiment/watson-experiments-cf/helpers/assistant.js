...
async function waitForAssistantsAvailability(client, assistants) {
  while (
    ...
    results.forEach((assistantDetails) => {
      if (!assistantDetails.value?.workspace_id) {
        console.log(JSON.stringify(assistantDetails));
      }
...
async function deleteAssistants(client, desiredAssistants) {
  desiredAssistants.forEach(async (assistant) => {
    await deleteAssistant(client, assistant.skillID);
  });
}
...
async function getSkill(apiKey, url, assistantId) {
  ...
  const res = await client.exportSkills({
    assistantId: assistantId,
  });

  if (res.result?.status === "Processing") {
    await sleep(1000);
    return await getSkill(client, assistantId);
  } else {
    return res.result;
  }
}
