...
async function createWorkspaceTable(conn) {
  ...
  try {
    conn.query(sql, (err) => {
      if (err) {
        if (err.sqlcode === 4136 || err.sqlcode === -601) {
          console.log("WORKSPACES table already existent!");
        } else {
          throw err;
        }
...
async function insertOnWorkspaceTable(conn, values) {
  var sql = `INSERT INTO CURATOR.WORKSPACES VALUES ('${values.skill.workspace_id}','${values.skill.name}','${values.rightInstance.guid}',NULL,NULL,'${values.rightInstance.account}','${values.rightInstance.credentials[0].credentials.apikey}','${values.rightInstance.credentials[0].credentials.url}');`;
...
async function checkIfRegisteredOnWorkspaceTable(conn, searchId) {
  return new Promise((resolve, reject) => {
    var sql = `select WORKSPACE_ID from CURATOR.WORKSPACES WHERE WORKSPACE_ID = '${searchId}';`;
    try {
